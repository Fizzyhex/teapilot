...
export function searchRepair(config: Config): string {
  return `Configuration: ${config.source?.directory ?? 'provided settings'}. Run teapilot setup${config.source ? ` --config-dir "${config.source.directory}"` : ''} and choose Reconfigure, then configure search. See usage/02-commands.md#web-research.`;
}
