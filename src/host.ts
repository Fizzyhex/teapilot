...
export interface HostRequest { prompt: string; cwd: string; web?: boolean; correction?: string; signal?: AbortSignal }
...
export async function runHost(config: Config, request: HostRequest, dependencies: HostDependencies): Promise<HostResult> {
  ...
  try {
    ...
    for (let index = 0; index <= config.policy.escalation.maxEscalations; index++) {
      ...
      if (!candidates.some(c => c.availability?.available)) return await finish(false, 'unavailable', previous?.text || 'No capability fits the configured availability and budget. Run npm run doctor.');
      dependencies.onProgress?.(scope ? `Routing escalation to ${scope.workload}.${scope.tier} (${previous?.reason}).` : 'Routing with JevRouter.');
      ...
      selected = decision.decision.selected ?? undefined;
      if (decision.status === 'no_decision' || !selected) return await finish(false, 'no_decision', `JevRouter did not authorize a route (${decision.fallback.type ?? 'manual_review'}). Review the receipt or clarify the request.`);
      const candidate = candidates.find(c => c.id === selected);
      const assessment = decision.decision.candidates.find(c => c.id === selected);
      if (!candidate || !candidate.availability?.available || !assessment || assessment.router.filtered || !assessment.router.allowed) return await finish(false, 'blocked', 'Selected capability did not pass the execution boundary.');
      if (decision.status === 'needs_confirmation' || assessment.router.requires_confirmation) {
