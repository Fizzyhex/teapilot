...
- **Only allowlisted Discord users can use it.** Messages from anyone else, and from bots, are ignored without a reply.
- **Where it listens:**
...
- **Access is the same as in the CLI.** Sessions start in `ask` mode at the configured repository root.
  ...
  - Approvals are **Approve / Deny** buttons that only allowlisted users can click. Unanswered approvals are denied after 10 minutes, and when teapilot stops.
...
4. Enter the Discord user IDs allowed to use teapilot. To copy one, enable Developer Mode (Settings → Advanced), right-click the user, and choose **Copy User ID**.
5. Optionally, enter a channel ID for @mentions. Press Enter for DMs only.
6. Choose the repository root. It defaults to `--cwd`.
7. Confirm, then open the printed invite link to add the bot to a server you share with the allowed users. Discord only delivers DMs from people who share a server with the bot.
...
| DMs get no reply | Check that your ID is allowlisted (`teapilot discord status`) and that you share a server with the bot. |
