...
The package smoke test installs an npm tarball in a temporary directory with Git disabled and exercises setup, config discovery, doctor, ask, and code without credentials. CI runs it on every supported platform/runtime. The separate `real Ollama validation` workflow downloads the default 9B and Cheap & Fast 4B Q8_0 presets and tests real streamed answers, tool continuation, and coding on Windows/Linux; it runs manually and as a mandatory release gate, not on pull requests. To run it against a local Ollama server: `npm run build`, then `npm run test:ollama` (downloads a model). `TEAPILOT_TEST_MODEL` selects `huihui_ai/qwen3.5-abliterated:9b`, `huihui_ai/qwen3.5-abliterated:35b-a3b-q4_K`, or `hf.co/mradermacher/Qwen3.5-4B-Uncensored-GGUF:Q8_0`. Validate the 35B preset on a machine with sufficient RAM.
