...
The first vertical slice is implemented. JevRouter's pinned TypeScript SDK handles routing, capability validation, availability, confidence, permissions/risk/confirmation, and receipt provenance. Teapilot owns execution, budgets, escalation, and local outcomes.
...
- Windows Node 24: type checking, 27 unit/integration tests, compilation, CLI startup, mock doctor and ask invocation.
- Linux Node 22.19.0: fresh Docker `npm ci`, the same 27 tests/checks and compiled startup.
...
- Search is snippet-based, opt-in, and requires a separately configured SearXNG service. No paid-provider/local-model quality assessment has been performed.
