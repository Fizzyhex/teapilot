...
describe('VS Code integration boundaries', () => {
  ...
  async function local(handler: Parameters<typeof mockServer>[0]) {
    ...
    f.config.models.local.baseUrl = server.url;
    f.config.models.local.contextTokens = 32768;
    return f;
  }
  ...
  it('streams a standalone model and settles its cost without executing tools', async () => {
    ...
    const events: any[] = [];
    const request = inferenceSchema.parse({ model: 'local', messages: [{ role: 'user', content: [{ type: 'text', text: 'write' }] }], tools: [{ name: 'write', description: 'write a file', parameters: { type: 'object', properties: {} } }], toolMode: 'required' });
    const result = await runInference(f.config, request, { approve: async () => true, onEvent: e => events.push(e) });
    expect(result).toMatchObject({ status: 'completed', spentUsd: 0, tier: 'local' });
  ...
  it('preserves tool call IDs and matching result names', async () => {
    const f = await fixture(); cleanups.push(f.cleanup);
    const request = inferenceSchema.parse({ model: 'local', messages: [
    ...
    const context = inferenceContext(request, f.config, 'local');
    ...
    expect(() => inferenceContext(request, f.config, 'local')).toThrow('Unmatched');
  });
  it('fixed models never fallback and unavailable models are not advertised', async () => {
    ...
    await expect(runInference(f.config, inferenceSchema.parse({ model: 'local', messages: [{ role: 'user', content: [{ type: 'text', text: 'hello' }] }] }), { approve: async () => true })).rejects.toThrow('Inference stopped');
    expect(calls).toBe(1);
